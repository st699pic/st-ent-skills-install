...
2. Inspect the repo README if needed.
3. Create a local `.env` or equivalent env export.
4. Verify the server starts with `node mcp/server.js`.
5. Register it with the target MCP client.
6. Run a smoke test (`tools/list` or one real call).
